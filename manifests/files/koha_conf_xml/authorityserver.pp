...
#
define koha::files::koha_conf_xml::authorityserver
(
	$ensure			= "present",
	$site_name		= $name,
	...
	$sru_host		= undef,
	$sru_port		= "9999",
	$sru_database		= $::koha::params::koha_conf_xml::config_authorityserver,
...
)
{
	...
	case $indexing_mode
	{
		"dom":	{ $_config = "${koha_site_dir}/${site_name}/zebra-authorities-dom.cfg" }
		"grs1":	{ $_config = "${koha_site_dir}/${site_name}/zebra-authorities.cfg" }
		default: { fail("invalid indexing mode for authority records '$indexing_mode'") }
	}

	$_dom_retrieval_info = pick($dom_retrieval_info, "${koha_config_dir}/${marc_format}-retrieval-info-auth-dom.xml")
	$_sru_explain = pick($sru_explain, "${koha_config_dir}/zebradb/explain-authorities.xml")
